...
# ====== НАСТРОЙКИ ======
TOKEN = os.environ.get("TOKEN")  # берем токен из Replit Secrets
...
def get_city_keyboard():
    keyboard = [
        [InlineKeyboardButton(f"🏙️ {city}", callback_data=f"city_{city}")]
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("city_"):
        ...
        keyboard = [
            [InlineKeyboardButton(f"📍 {district}", callback_data=f"district_{district}")]
    ...
    if data.startswith("district_"):
        ...
        if not products:
            ...
            return
        keyboard = [
            [InlineKeyboardButton(f"{p['name']} | від {p['price']}₴", callback_data=f"product_{district}_{i}")]
            for i, p in enumerate(products)
        ]
        keyboard.append([InlineKeyboardButton("🏠 ГОЛОВНЕ МЕНЮ", callback_data="menu")])
        await query.message.delete()
        await query.message.reply_text(
            f"🛍️ *Доступні товари в {district}*",
            reply_markup=InlineKeyboardMarkup(keyboard),
            parse_mode=ParseMode.MARKDOWN
        )
        return
